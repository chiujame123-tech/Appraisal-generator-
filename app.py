...
# ==========================================
# TAB 2: 舊料資料庫 (Full Text & Year Filter)
# ==========================================
with tab2:
    ...
    if region == "🏢 九龍 (Kowloon)":
        if year_selected == "2025":
            ...
            在局內工作方面，吳隊目極之能幹可靠，會協助管理局內各形各色的日常事務，在隊員的操練方面，他亦會主動向主管提出一些操練上的建議，對年資較淺的隊員的滅火及救援訓練要求甚高，每一次的局內操練，都會親力親為，就不同的情況例如高空拯救、一級火警、交通意外有人被困等，都會悉心設計出與真實環境相似的訓練，務求令到隊員透過操練，維持他們對前線工作的敏感度及效率。此外他會聯同其他隊目及總隊目，主動帶領年資較淺的隊員，為他們工作上遇到的問題提供意見，協助他們適應本局忙碌及充實的工作環境。
    ...
    if st.button("🖍️ 開始標記分析", type="primary"):
        if text_to_highlight:
            ...
            for i in range(0, len(parts)-1, 2):
                ...
                if matched_item:
                    # 使用 HTML Ruby tag 畫紅線同標數字
                    # text-decoration-color 控制底線顏色，rt 控制上方數字
            ...
            st.info("💡 提示：句子上面嘅紅色數字代表對應嘅 FS-278 項目（例如 1 = 工作認識，6 = 服從紀律）。因為係關鍵字辨識，未必100%完美，但足夠快速睇得出文章重點分佈！")
        else:
            st.warning("請先輸入考績文章！")
